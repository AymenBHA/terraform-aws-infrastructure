...
Terraform remote state provides a single source of truth for infrastructure.

The state file is stored in S3, and DynamoDB locking prevents multiple Terraform operations from modifying the state at the same time.

---

## What happens if the state file is deleted?

The AWS resources continue running, but Terraform loses information about the resources it manages.

The state must be restored from S3 version history or rebuilt using Terraform import.
...
- Use state locking
- Separate backend infrastructure from application infrastructure
